...
class ExpenseRepository {
  ...
  async updateTagInExpense(id, oldTag, newTag) { 
    const db = getDB();
    const oldtag = {_id : new ObjectId(id) , tags : oldTag }
    const newtag = {$set : {"tag.$" : newTag}};
    const result = await db.collection.updateOne(oldtag , newtag);
    return result;
  }
  ...
  async deleteTagFromExpense(id, tag) { 
    const db = getDB;
    const tagname = {_id : new ObjectId(id)}
    const remove = { $pull: { tags: tag } };
    return await db.collection(this.collection).updateOne(tagname , remove);
  }
